...
// Injeta os HTMLs dos pneus via código para não travar a tela
window.onload = function() {
    let construtorFicha = arr => arr.map(p => `
        <details><summary>${p.n}</summary>
...
function salvarFichaNaNuvemBackground() {
    let payload = {
        ...
        tipo: document.getElementById('tipo-veiculo').value,          // NOVO
        motorista: document.getElementById('nome-motorista').value,   // NOVO
...
function alternarEdicaoHeader() { 
    // INCLUINDO OS NOVOS CAMPOS NA LIBERAÇÃO DO BOTÃO "EDITAR"
...
function calcularTacografo() { 
    ...
    let p = s.split('-'); let px = new Date(p[0], p[1] - 1, p[2]); let ul = new Date(px); ul.setFullYear(ul.getFullYear() - 2); 
...
function alternarEdicaoEquip() { 
    let c = [document.getElementById('qtd-carrinhos'), document.getElementById('qtd-cones')]; let b = document.getElementById('btn-editar-equip'); 
    ...
    else { c.forEach(x => { x.setAttribute('readonly', 'true'); x.classList.add('travado'); }); b.innerHTML = "✏️ Editar"; b.style.backgroundColor = "transparent"; b.style.color = "#1a4d2e"; salvarFichaNaNuvemBackground(); } 
}
...
function calcularGraxa() { 
    ...
    let p = s.split('-'); let ultima = new Date(p[0], p[1] - 1, p[2]); let proxima = new Date(ultima); proxima.setDate(proxima.getDate() + 30); 
...
function abrirDocPDF() { 
    if (urlDocAtual && urlDocAtual.trim() !== "") window.open(urlDocAtual, '_blank'); else alert("Nenhum documento cadastrado para este veículo."); 
}

// ABASTECIMENTO
function mudarFormAbast() { 
    let t = document.getElementById('tipo-abast').value; 
    if (t.includes("CHEGADA")) { document.getElementById('form-abast-veiculo').style.display = 'none'; document.getElementById('form-abast-chegada').style.display = 'block'; } 
    else { document.getElementById('form-abast-veiculo').style.display = 'block'; document.getElementById('form-abast-chegada').style.display = 'none'; } 
}

function preencherDataHoraAbast() { 
    let n = new Date(); let hL = new Date(n.getTime() - (n.getTimezoneOffset() * 60000)).toISOString().slice(0,16); 
    let eA = document.getElementById('abast-data'); if(eA) eA.value = hL; let eC = document.getElementById('chegada-data'); if(eC) eC.value = hL; 
    document.getElementById('estoque-diesel-geral').innerText = window.estoqueDiesel + " L"; document.getElementById('estoque-arla-geral').innerText = window.estoqueArla + " L"; document.getElementById('gasto-mes-geral').innerText = window.gastoMesGeral + " L"; 
}
...
function renderizarHistorico(d) { 
    let c = document.getElementById('container-historico'); c.innerHTML = ""; 
    if (!d || d.length === 0) { c.innerHTML = "<p style='text-align:center; color:#666;'>Nenhum checklist registrado ainda.</p>"; return; } 
    d.forEach(i => { let v = document.createElement('div'); v.className = "historico-item"; v.innerHTML = `<span style="font-weight: bold; color: #1a4d2e;">${i.placa}</span> <span style="color: #555; font-size: 12px;">${String(i.data).split('T')[0]} ✅</span>`; c.appendChild(v); }); 
}
...
function iniciarNovoChecklist() { 
    ...
    if(cF && aF) { if (ve && ve.mes_foto === mA) { cF.style.display = 'none'; aF.style.display = 'block'; } else { cF.style.display = 'block'; aF.style.display = 'none'; } } 
    
    // INJETANDO OS VALORES NOVOS AQUI
