/* =========================================================
   SISTEMA LINCE - CÉREBRO LOCAL (ALTA PERFORMANCE)
   ========================================================= */
...
window.isCavalo = false; let urlDocAtual = ""; window.frota = {}; 
...
// Assim que a tela abre, ele "costura" o HTML dos Pneus de forma 100% segura
window.onload = function() {
    let construtorFicha = arr => arr.map(p => `<details><summary>${p.n}</summary><div class="pneu-detalhes"><div class="linha-info"><span class="info-label" style="margin:0;">Estado:</span> <span class="info-valor" id="estado-${p.id}" style="font-weight:bold;">---</span></div><div class="linha-info"><span class="info-label" style="margin:0;">TWI:</span> <span class="info-valor" id="twi-${p.id}">---</span></div><div class="linha-info" style="margin-top:10px;"><span class="info-label" style="margin:0;">KM Troca:</span> <input type="number" id="km-troca-${p.id}" class="input-editavel travado" value="0" readonly></div><div class="linha-info" style="margin-top:5px;"><span class="info-label" style="margin:0;">Data Troca:</span> <input type="date" id="data-troca-${p.id}" class="input-editavel travado" readonly></div><div class="linha-info" style="margin-top:5px; border-top:1px dashed #ccc; padding-top:5px;"><span class="info-label" style="margin:0;">Próx Rodízio:</span> <span class="info-valor" id="prox-rod-${p.id}">---</span></div><div class="linha-info"><span class="info-label" style="margin:0;">Status Rodízio:</span> <span id="status-rod-${p.id}" style="font-weight:bold;">---</span></div></div></details>`).join('');
    let ctCavalo = document.getElementById('ficha-pneus-cavalo-container'); if(ctCavalo) ctCavalo.innerHTML = construtorFicha(pneusCavalo);
    let ctCarreta = document.getElementById('ficha-pneus-carreta-container'); if(ctCarreta) ctCarreta.innerHTML = construtorFicha(pneusCarreta);

    let construtorChk = arr => arr.map(p => `<div style="display: flex; align-items: center; justify-content: space-between; gap: 8px; border-bottom: 1px dashed #ccc; padding-bottom: 5px;"><span style="font-weight: bold; width: 50px; color:${arr===pneusCarreta?'#0056b3':'#333'};">${p.n}</span><input type="number" id="chk-twi-${p.id}" placeholder="mm" class="input-campo" style="margin:0; width: 80px; padding: 10px; text-align:center;" oninput="calcularStatusTwi(this, 'badge-estado-${p.id}')"><span id="badge-estado-${p.id}" class="twi-estado-badge" style="flex: 1;">Aguardando...</span></div>`).join('');
    let chkCavalo = document.getElementById('chk-pneus-cavalo-container'); if(chkCavalo) chkCavalo.innerHTML = construtorChk(pneusCavalo);
...
async function fazerLogin() {
    ...
    try {
        ...
        if (res1.sucesso) {
            ...
            if (res2.sucesso) {
                window.frota = res2.frota; window.estoqueDiesel = res2.estoque_diesel; window.estoqueArla = res2.estoque_arla; window.gastoMesGeral = res2.gasto_mes_geral;
                renderizarHistorico(res2.historico); esconderTodasTelas(); document.getElementById('tela-placas').style.display = 'flex';
...
function selecionarPlaca(placa) {
    ...
    try {
        ...
        let kTroca = 15000; if (dados.km_oleo) kTroca = parseInt(String(dados.km_oleo).replace(/\./g, '')) || 15000;
        ...
        if (iL && pL) { if (dados.foto_lateral) { iL.src = dados.foto_lateral; iL.style.display = 'block'; pL.style.display = 'none'; } else { iL.style.display = 'none'; pL.style.display = 'flex'; } }
        let iT = document.getElementById('ficha-img-tras'); let pT = document.getElementById('ficha-pl-tras');
        if (iT && pT) { if (dados.foto_traseira) { iT.src = dados.foto_traseira; iT.style.display = 'block'; pT.style.display = 'none'; } else { iT.style.display = 'none'; pT.style.display = 'flex'; } }
...
function salvarFichaNaNuvemBackground() {
    let p = { acao: "salvar_ficha_tecnica", placa: document.getElementById('texto-placa-interna').innerText, km_atual: document.getElementById('km-master').value, km_oleo: document.getElementById('km-proxima-troca').value, data_tacografo: document.getElementById('data-proxima-afericao').value, data_graxa: document.getElementById('data-engraxada').value, qtd_carrinhos: document.getElementById('qtd-carrinhos').value, qtd_cones: document.getElementById('qtd-cones').value, pneus: {} };
    let tIds = ['dd','de','tde','tdi','tee','tei','tkde','tkdi','tkee','tkei','1step','c1','c2','c3','c4','c5','c6','c7','c8','c9','c10','2step'];
    tIds.forEach(id => { let km = document.getElementById('km-troca-'+id); let dt = document.getElementById('data-troca-'+id); if(km && dt) { p.pneus[id] = { km_ultima_troca: km.value, data_ultima_troca: dt.value }; } });
    fetch(API_URL, { method: 'POST', body: JSON.stringify(p) }).then(res => res.json()).then(d => {
        if(d.sucesso) {
            window.frota[p.placa].km_atual = p.km_atual; window.frota[p.placa].km_oleo = p.km_oleo; window.frota[p.placa].data_tacografo = p.data_tacografo; window.frota[p.placa].data_graxa = p.data_graxa; window.frota[p.placa].qtd_carrinhos = p.qtd_carrinhos; window.frota[p.placa].qtd_cones = p.qtd_cones;
            for(let id in p.pneus) { if(!window.frota[p.placa].pneus[id]) window.frota[p.placa].pneus[id] = {}; window.frota[p.placa].pneus[id].km_ultima_troca = p.pneus[id].km_ultima_troca; window.frota[p.placa].pneus[id].data_ultima_troca = p.pneus[id].data_ultima_troca; }
            let elAv = document.getElementById('aviso-ultimo-km'); if(elAv) elAv.innerText = p.km_atual;
        }
    });
}
...
function calcularOleo() { let kmA = parseInt(document.getElementById('km-master').value) || 0; let kmP = parseInt(document.getElementById('km-proxima-troca').value) || 0; let kmF = kmP - kmA; let txt = document.getElementById('status-oleo'); if (kmF <= 0) { txt.innerHTML = `VENCIDO (${Math.abs(kmF)} KM) ❌`; txt.style.color = "red"; } else if (kmF <= 1500) { txt.innerHTML = `Faltam ${kmF} KM ⚠️`; txt.style.color = "#d4a017"; } else { txt.innerHTML = `Faltam ${kmF} KM ✅`; txt.style.color = "green"; } }
...
function calcularGraxa() { let s = document.getElementById('data-engraxada').value; let t = document.getElementById('status-graxa'); if (!s || s.length < 8) { if(t) t.innerHTML = "---"; document.getElementById('data-prox-engraxada').innerText = "--/--/----"; return; } let p = s.split('-'); let u = new Date(p[0], p[1] - 1, p[2]); let px = new Date(u); px.setDate(px.getDate() + 30); document.getElementById('data-prox-engraxada').innerText = `${String(px.getDate()).padStart(2,'0')}/${String(px.getMonth()+1).padStart(2,'0')}/${px.getFullYear()}`; let h = new Date(); h.setHours(0,0,0,0); let d = Math.ceil((px.getTime() - h.getTime()) / (1000 * 3600 * 24)); if (d < 0) { t.innerHTML = `VENCIDO há ${Math.abs(d)} dias ❌`; t.style.color = "red"; } else if (d <= 5) { t.innerHTML = `Atenção: Faltam ${d} dias ⚠️`; t.style.color = "#d4a017"; } else { t.innerHTML = `Faltam ${d} dias ✅`; t.style.color = "green"; } }
...
async function salvarAbastecimentoNuvem() {
    ...
    if (t.includes("CHEGADA")) {
        ...
        p.data = document.getElementById('abast-data').value; p.km = parseFloat(document.getElementById('abast-km-novo').value) || 0; p.litros = parseFloat(document.getElementById('abast-litros-bomba').value) || 0; p.motorista = document.getElementById('abast-motorista').value; p.responsavel = document.getElementById('abast-resp').value; p.nf = "";
    ...
    let b = document.getElementById('btn-salvar-abast-nuvem'); b.innerText = "Salvando... ⏳"; b.disabled = true;
    try {
...
function processarFoto(input, idPreview) {
    ...
    r.onload = function(e) {
        ...
        img.onload = function() {
            const cv = document.createElement('canvas'); const MW = 800; let w = img.width; let h = img.height;
            if (w > MW) { h = Math.round((h * MW) / w); w = MW; } cv.width = w; cv.height = h;
            const cx = cv.getContext('2d'); cx.drawImage(img, 0, 0, w, h);
            const dU = cv.toDataURL('image/jpeg', 0.6); 
            if(idPreview === 'preview-lat') b64Lateral = dU; if(idPreview === 'preview-tras') b64Traseira = dU;
...
function iniciarNovoChecklist() { let pl = document.getElementById('texto-placa-interna').innerText; document.getElementById('lista-historico-checklist').style.display = 'none'; document.getElementById('form-novo-checklist').style.display = 'block'; document.querySelectorAll('input[type="checkbox"]').forEach(c => c.checked = false); document.querySelectorAll('input[id$="-outro"]').forEach(c => { c.value = ""; c.style.display = 'none'; }); document.querySelectorAll('input[id^="chk-twi-"]').forEach(i => i.value = ""); document.querySelectorAll('.twi-estado-badge').forEach(b => { b.innerText = "Aguardando..."; b.style.backgroundColor = "#eee"; b.style.color = "#666"; }); b64Lateral = ""; b64Traseira = ""; let lI = document.getElementById('foto-lat-input'); if(lI) lI.value = ""; let tI = document.getElementById('foto-tras-input'); if(tI) tI.value = ""; let pL = document.getElementById('preview-lat'); if(pL) pL.style.display = "none"; let pT = document.getElementById('preview-tras'); if(pT) pT.style.display = "none"; let mA = new Date().toISOString().slice(0, 7); let ve = window.frota[pl]; let cF = document.getElementById('container-inputs-fotos'); let aF = document.getElementById('aviso-fotos-ok'); if(cF && aF) { if (ve && ve.mes_foto === mA) { cF.style.display = 'none'; aF.style.display = 'block'; } else { cF.style.display = 'block'; aF.style.display = 'none'; } } document.getElementById('chk-placa').value = pl; document.getElementById('chk-motorista').value = document.getElementById('nome-motorista').value; document.getElementById('chk-km').value = document.getElementById('km-master').value; document.getElementById('chk-km-oleo').value = document.getElementById('km-proxima-troca').value; document.getElementById('chk-data-taco').value = document.getElementById('data-proxima-afericao').value; document.getElementById('chk-data-graxa').value = document.getElementById('data-engraxada').value; mudarAbaChecklist(1); }
...
function avancarPasso(px) {
    let pA = px - 1; 
    if (pA === 1) { if(window.isCavalo && !document.getElementById('chk-placa-carreta').value) return alert("❌ Como você está com um CAVALO, selecione a Placa da Carreta!"); if(!document.getElementById('chk-modelo').value) return alert("❌ Selecione o Modelo!"); if(!document.getElementById('chk-motorista').value) return alert("❌ Digite o Motorista!"); if(!document.getElementById('chk-km').value) return alert("❌ Digite o KM Atual!"); }
    if (pA === 2) { let gM = ['chk-motor', 'chk-cambio', 'chk-embreagem', 'chk-direcao', 'chk-freios', 'chk-suspensao']; for (let g of gM) { if (document.querySelectorAll(`input[name="${g}"]:checked`).length === 0) return alert(`❌ Você esqueceu de preencher uma das sessões de Mecânica!`); } if (window.isCavalo) { if (!document.getElementById('chk-asa').value) return alert("❌ Responda Asa Delta!"); if (!document.getElementById('chk-freio-lona').value) return alert("❌ Responda Freio Lona!"); if (!document.getElementById('chk-faixas').value) return alert("❌ Responda Faixas Refletivas!"); } }
    if (pA === 3) { let gE = ['chk-pneus_geral', 'chk-eletrica', 'chk-indicadores', 'chk-cabine']; for (let g of gE) { if (document.querySelectorAll(`input[name="${g}"]:checked`).length === 0) return alert(`❌ Você esqueceu de preencher uma das sessões de Cabine/Elétrica!`); } }
...
async function enviarChecklist() {
    ...
    try {
        ...
        if (d.sucesso) {
            ...
            window.frota[pl.placa].km_atual = pl.km_atual; window.frota[pl.placa].km_oleo = pl.km_oleo;
